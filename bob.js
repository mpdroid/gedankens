...
class Bob extends Object3D {
  ...
  stopWalking() {
    resetArm(this.leftArm);
    resetArm(this.rightArm);
    resetLeg(this.leftLeg);
    resetLeg(this.rightLeg);
  }
...
function resetArm(arm) {
  arm.skeleton.bones.forEach(bone => {
    bone.rotation.z = 0;
  });
}
...
function resetLeg(leg) {
  leg.skeleton.bones.forEach(bone => {
    bone.rotation.z = 0;
  });
}


function oscillateLeg(leg, direction = 0, time) {
  const sinval = Math.sin(time);
  for (let i = 0; i < leg.skeleton.bones.length; i++) {
    if ((direction == 0 && sinval < 0) || (direction > 0 && sinval > 0)) {
      leg.skeleton.bones[i].rotation.z = sinval / leg.skeleton.bones.length;

    } else {
      if (i === 0) {
        leg.skeleton.bones[i].rotation.z = sinval / 2;
      }
